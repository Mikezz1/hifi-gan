...
class BaseDataset(Dataset):
    def __init__(
        ...
        index = self._filter_records_from_dataset(
            index, max_audio_length, config_parser["dataset"]["limit"]
        )
        ...
        self.melspec_transform = MelSpectrogram(self.config_parser)
    ...
    def adjust_audio_len(self, audio):
        """FIX"""
        hl = self.config_parser["preprocessing"]["hop_length"]
    ...
    def process_wave(self, audio_tensor_wave: Tensor):
        with torch.no_grad():
            audio_tensor_spec = self.melspec_transform(audio_tensor_wave)
